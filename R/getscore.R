...
getscore = function(score = "CFS1",index_gene_up, index_gene_down, subcounts){
  ...
  if(score == "CFS2"){
    if(length(index_gene_up) == 0){
      temp.m.u = rep(0,n)}else{
        ...
        temp.m.u[temp.m.u <= 0.5] = 0
        temp.m.u = temp.m.u / (length(index_gene_up) + length(index_gene_down))
      }
    if (length(index_gene_down) == 0){
      temp.m.d = rep(0,n)}else{
        temp.m.d = subcounts[index_gene_down,]
        ...
        temp.m.d[temp.m.d <= 0.5] = 0
        temp.m.d = temp.m.d / (length(index_gene_up) + length(index_gene_down))
      }
